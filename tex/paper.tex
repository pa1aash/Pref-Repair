...
\title{GARP-Blind Perturbations Outperform Minimal GARP Repair Under an Exogenous Payoff}
...
An LLM agent makes a sequence of budget-constrained choices over two goods. We compute, as a
single mixed-integer linear program, the minimal quantity perturbation that would make the
agent's own realized choice sequence GARP-consistent, applied post hoc to a frozen agent: nothing
...
that dose is not confounded with model capacity or training. A 100-session pilot (2 models
$\times$ 2 framings $\times$ 25 seeds) preceded the main experiment. We run this at a model scale
where the pilot found coherence headroom (1.5B parameters), and repeat the same pipeline at 3B, where the
pilot found mean baseline CCEI (Critical Cost Efficiency Index; Afriat, 1972, 1973) 0.99. The 3B
run is an identification control on the method. The two
scales are not two points on a shared dose curve.

The reciprocal-price framing manipulation, intended to induce coherence variation at 1.5B,
produced in the pilot a GARP pass-rate collapse from 0.76 to 0.17 at 3B under the same
manipulation. The pilot's separate CCEI estimate at 1.5B ($+0.0169$, $p=0.66$) was never
statistically significant, and the main experiment's retry-corrected estimate at 1.5B
($-0.0109$, $p=0.73$) is likewise indistinguishable from zero (\S\ref{sec:results-framing}). The
discard rate is the more reliable signal that the manipulation disrupted the 1.5B agent (our
claim C3): it is measured without conditioning on completion, and the CCEI point estimate is
not; 52\% of the pilot's reciprocal-framing sessions at 1.5B were silently discarded for failing
the output-format contract. Replacing single-turn
...
protocol and reported per cell. (5) The single-turn-vs-multi-turn elicitation-format
manipulation above, which runs opposite to an independently published finding in the same
domain and model family at a larger scale \citep{wang2025tactics}.
...
agent and is evaluated on forecasting, one operating on synthetic-data-trained forecasts of human
survey consumers' choices, not an LLM agent's own decisions.

\textbf{Preference-model variants.} A parallel line varies the transitivity of the
\emph{preference model} rather than an agent's choices, confounding coherence with capacity by
construction: GPM \citep{zhang2025gpm} loses on its own length-controlled metric in 18 of 24
cells, and HRC/DSPPO's \citep{huang2026hrc} inverted-U dose--response curve is over a
training-time schedule weight scored by an LLM judge, not an exogenous payoff. Ours acts post hoc
on a payoff that is exogenous by construction. \citet{andrews2026revealed} proposes
$1-\mathrm{CCEI}$ as a training penalty on a priori grounds and reports no measurement of it; we
report the measurement that proposal calls for.

\textbf{Psychometric reliability.}
\citet{nitsch2022reliability} find that CCEI and Houtman--Maks \citep{houtman1985determining}
test--retest reliability (ICC) ranges from 0.158 to 0.685 across eight datasets, never reaching
the conventional ``good'' threshold of 0.75, and that mean CCEI fell among
...
The MILP formulation below is \citet{demuynck2023computing}'s multiplier-free ordinal
characterization of minimal-quantity-error GARP repair, applied here to an LLM agent's own choice
sequence. For a sequence of $T$ observations $(p_t, x_t)_{t=1}^T$, $G$ goods, GARP holds if there is no
...
$T=25$ and $G=2$ this is 600 binaries and 125 continuous variables per trace, solved as a single
...
distance; it is not supplied to the solver (Appendix~\ref{app:payoff-audit}, check~1). Because the
GARP-consistent set is a union of polyhedra rather than a single convex set, we claim no
distance-minimization guarantee analogous to a convex-cone projection \citep{wang2026poise}.
...
conditions, models, and repairs. This has the structure of a money-metric utility index, the
standard welfare-comparison device over the same GARP/Afriat objects this paper's method already
relies on. \citet{ouyang2024aidecisionmaker} give a comparable exogenous, real-world payoff
(LLM-generated forecasts scored against firms' realized future spending, finding a
\emph{non-monotonic} alignment-accuracy relationship), but lack a placebo of matched size to
separate an alignment-specific effect from a generic fine-tuning effect; our design makes that
separation.
...
every check and number). A fifth concern, that this payoff's concavity could reward displacement
alone, motivates the control constructed next.
...
bundle's payoff, provided the displacement does not overshoot the optimum, whether or not it
restores GARP-consistency. We construct a \emph{null operator}
...
matches the real projection's dose. The null does not call the GARP check. It restores consistency
in none of the 85 traces; it is scored by the payoff of \S\ref{sec:method-payoff}.
...
$\alpha_i\sim\mathrm{Uniform}(0.05,0.95)$, drawn independently for each of the 85 traces $i$ and
each of $K=20$ replicate draws, read only at scoring time. We report two null constructions under
this payoff: a \emph{primary} null, unaware of $\alpha_i$, and an
\emph{oracle} null that aims at the true per-trace optimum $x^*_{\alpha_i}$. The two are reported
separately. The oracle null is an upper bound: it uses the per-trace $\alpha_i$, which a repair
algorithm does not observe.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.74\linewidth]{../figures/fig_pipeline_schematic.pdf}
...
Each of $N=30$ independent replicates per (model, condition) cell draws a fresh $T=25$, $G=2$
budget set (prices $p_A,p_B\sim U[0.1,1.0]$ i.i.d., rejected unless $\max(p_A,p_B)\ge0.5$, budget
exhaustion enforced), seeded independently to support between-condition replication. A
\emph{slot} is one such design position; each slot is run for up to three \emph{attempts}, one
session each (\S\ref{sec:discard}); a completed attempt yields one \emph{trace}. Continuous,
independently drawn prices with enforced budget exhaustion also avoid a pathology documented by
\citet{andrews2026revealed}: CCEI can read exactly 1.0 on GARP-violating data whenever a
comparison lands on exact budget-line equality, common under grid-sampled or rounded prices.
$N=30$ gives power $\ge0.999$ against the pilot's full framing-effect magnitude on the pass-rate
metric and $\ge0.80$ down to an effect attenuated to $\sim$60\% of that magnitude (pass rate
0.76$\to$0.40, vs.\ the pilot's 0.76$\to$0.17), but is underpowered for a CCEI-scale effect of the
pilot's magnitude at 1.5B (minimum detectable $\approx0.11$). Every replicate's Bronars power
\citep{bronars1987power} is recorded; all 150 draws held Bronars power $\ge0.998$.
...
required output format and were silently discarded under the pilot's handling --- standard
practice, whose consequences this paper measures as an instrument-validity claim. The main
experiment implements a capped retry protocol: a session failing the $\ge20/25$-valid-rounds
threshold is retried up to three times with a new random seed; a slot still failing after three
attempts is a residual discard, excluded from the main CCEI/GARP analysis but scored separately
for the audit in Table~\ref{tab:discardbreakdown} (\S\ref{sec:results-framing}). Three of the 30
...
scales, and the partial correlation of dose against $\Delta$payoff$_{\mathrm{real}}$, controlling
for the null's mechanically predicted gain, remains positive and significant at both scales
separately ($r=0.574$ pooled, $p=1.13\times10^{-8}$; per-model breakdown in
Table~\ref{tab:permodel}, Appendix~\ref{app:payoff-audit}), so dose retains information beyond
...
$\alpha_i\sim\mathrm{Uniform}(0.05,0.95)$ per trace, over $K=20$ independent draws
(\S\ref{sec:method-nullop}). Experiment 2 reproduces Experiment 1: the primary null still
outperforms the real repair, by $3.0\times$ (mean $\Delta$payoff$_{\mathrm{null}}=0.02167$ vs.\
mean $\Delta$payoff$_{\mathrm{real}}=0.00723$, Wilcoxon $p=1.24\times10^{-5}$, win rate 70.8\%),
significant in 20 of 20 draws and in both models separately. The oracle null, privileged to know
the true per-trace target, wins 80.4\% of traces (mean $\Delta$payoff$_{\mathrm{null}}=0.02224$,
$p=1.35\times10^{-9}$, significant in 20 of 20 draws, also for both models separately). The
partial correlation of dose with $\Delta$payoff$_{\mathrm{real}}$, controlling for the null,
attenuates from Experiment 1's $r=0.574$ to a mean $r=0.37$ across the 20 draws (SD $0.227$
primary-null, $0.223$ oracle-null), significant in 14 of 20 (primary) and 15 of 20 (oracle)
draws; the attenuation and the weaker partial correlation with the real repair's own gain both
hold per model, more sharply for qwen2.5:1.5b (Table~\ref{tab:permodel}).
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\linewidth]{../figures/fig_mechanism_illustration.pdf}
  \caption{The largest-dose trace (qwen2.5:1.5b, reciprocal, replicate 12; $L_1=111.64$,
  $T=21$ rounds). With $G=2$, payoff is a function of expenditure share $s$.
  \textbf{(A)} The original payoff $2\sqrt{s(1-s)}$, peak at $s=0.5$;
  \textbf{(B)} the corrected payoff at this trace's draw $\alpha_i=0.704$;
...
At 1.5B, the reciprocal-framing effect does not survive correction for discard-selection.
First-attempt discard under reciprocal framing was 43.3\% (13/30), against the pilot's unretried
52\%, falling to a residual 20.0\% (6/30) after the retry protocol; it produces more discards
than any other condition, including the other 1.5B manipulation (multi-turn, 0\%). Once the
...
experiment detects a CCEI shift, as the pilot's $+0.0169$ ($p=0.66$) and the main experiment's
$-0.0109$ ($p=0.73$) are two statistically indistinguishable nulls.

Retry-rescued sessions differ from first-attempt sessions: mean CCEI 0.9651 against
...
the selection concern shows up more sharply at 1.5B than at 3B. Table~\ref{tab:discardbreakdown}
breaks the discards down by attempt.
...
\textbf{CCEI is underpowered at 1.5B.} Baseline WSCV (Within-Subject Coefficient of Variation)
$14.8\%$ is comparable to published human test--retest noise \citep{nitsch2022reliability}, and
$N=30$ is underpowered for a CCEI-scale effect of the pilot's magnitude (minimum detectable
$2.5\times$ the pilot's estimate; a CCEI-power-matched design would need $N=111$--$161$, which we
did not run). The pass-rate metric is powered $\ge0.999$ throughout, and every CCEI result
carries its own confidence interval.

\textbf{Single run per condition.} Each (model, condition) cell is one run of the stated
protocol: we report no between-run variance on the headline statistics, only the within-run
replicate variance already in the reported confidence intervals and $p$-values.

\textbf{One scale point per model.} The two-model design isolates the identification strategy
from a scale confound but does not trace a dose--response relationship \emph{across} scale: in
our model-selection sweep (\S\ref{sec:design}), coherence headroom and measurement reliability
did not coexist in any model tested, and a third model family was excluded for compatibility
reasons.

\textbf{Two exogenous payoffs.} The first payoff's single, globally identical optimum is
exploitable by a GARP-blind operator; the second (\S\ref{sec:method-payoff},
\S\ref{sec:method-nullop}) removes that property and reproduces the same verdict. A third
payoff, chosen after seeing the second was also negative, would be payoff-shopping --- the
researcher-degree-of-freedom problem an exogenous payoff rules out (\S\ref{sec:results-c1}).

\textbf{Consistent with three prior negative results.} Three independently published repair
attempts moved the wrong way (\S\ref{sec:related}); our null-operator result
(\S\ref{sec:results-c1}) is a fourth. Separately, our reciprocal-framing manipulation at 1.5B
(\S\ref{sec:results-framing}) failed to induce the coherence variation it targeted. That leaves
C2 open; it is not evidence the relationship is negative in general.

\section{Broader Impacts}

This result argues against imposing GARP repair on AI economic agents as a routine intervention
without a matched control: under two payoff designs, a GARP-blind operator outperforms the real
repair (\S\ref{sec:results-c1}), so a practitioner relying on a raw within-scale dose--response
correlation alone would act on the confounded estimate this paper's control was built to
separate. Correcting discard selection removes the reciprocal-framing manipulation's apparent
coherence effect (\S\ref{sec:results-framing}); the design measures whether repair helps, and
here it does not. This instrument-validity finding (\S\ref{sec:discard}) is not specific to our
instrument: any consistency instrument that drops disruption-caused failures without recording
them will underestimate the disrupting manipulation. We see no elevated misuse risk specific to
this work: synthetic tasks, open-weight models, no new dataset, no human subjects.

\section{Conclusion}
\label{sec:conclusion}

Restoring GARP-consistency in an LLM agent's own realized choices does not buy more exogenous
payoff than displacement alone: at both model scales and under two payoff designs, a
dose-matched, GARP-blind null operator outperforms the real repair on the same payoff. C1 is not
supported. Trace extremity predicts the null's advantage in both experiments, a candidate
explanation that does not establish the mechanism. C2, the coherence--competence question,
remains open --- this is the first attempt we know of to measure it without confounding coherence
with capacity, a preference-judgment outcome, or displacement magnitude --- and a practitioner
evaluating coherence repair should include a dose-matched null operator before reading a
dose--response correlation as evidence of benefit. Two further findings stand on their own: a
framing manipulation's apparent effect was a discard-selection artifact (C3), and an
elicitation-format manipulation moved GARP pass rate opposite to \citet{wang2025tactics}'s
finding in the same domain and model family at a larger scale.
...
(Pearson $r=-0.411$, $p=9.4\times10^{-5}$), and worse-starting traces improve more
($r=-0.415$, $p=8.0\times10^{-5}$, between raw payoff and $\Delta$payoff). These are two distinct
correlations that round to the same $-0.41$; neither is a restatement of the other. This is a
ceiling-effect confound. The
...
\textbf{(5) Per-model breakdown of the null-operator comparison.} Table~\ref{tab:permodel} gives
the numbers pooled in \S\ref{sec:results-c1}, split by model.

\begin{table}[h]
\centering
\footnotesize
\caption{Per-model breakdown of the null-operator comparison (\S\ref{sec:results-c1}).
Experiment 1 uses the original payoff. Experiment 2 uses the corrected payoff, aggregated over
$K=20$ draws (mean values; ``sig.'' counts are draws with Wilcoxon $p<0.05$ out of 20). Partial
$r$ is the partial correlation of dose against $\Delta$payoff$_{\mathrm{real}}$ controlling for
the null's predicted gain (mean and, for Experiment 2, its significance count).}
\label{tab:permodel}
\begin{tabular}{llrrrr}
\toprule
Experiment & Model & $n$ & $\Delta$payoff$_{\mathrm{real}}$ & $\Delta$payoff$_{\mathrm{null}}$ & partial $r$ \\
\midrule
1, real null      & llama3.2:3b  & 25 & 0.0039  & 0.0105 ($p=0.0010$)              & 0.750 \\
1, real null      & qwen2.5:1.5b & 60 & 0.0113  & 0.0267 ($p=5.5{\times}10^{-8}$)  & 0.567 \\
\midrule
2, primary null   & llama3.2:3b  & 25 & 0.00276 & 0.00906 (16/20 sig.)             & 0.553 (18/20 sig.) \\
2, primary null   & qwen2.5:1.5b & 60 & 0.00910 & 0.02693 (20/20 sig.)             & 0.341 (12/20 sig.) \\
\midrule
2, oracle null    & llama3.2:3b  & 25 & 0.00276 & 0.00985 (20/20 sig.)             & --- \\
2, oracle null    & qwen2.5:1.5b & 60 & 0.00910 & 0.02741 (20/20 sig.)             & --- \\
\bottomrule
\end{tabular}
\end{table}
...
minimum. Re-solving at $\gamma \in \{10^{-2}, 10^{-3}, 10^{-4}, 10^{-5}, 10^{-6}\}$ on a
dose-stratified sample of 17 of the 85 GARP-violating traces (spanning the full observed dose
range, $0.17$--$111.64$) changes the reported $L_1$ distance by at most $16.8\%$ relative to the
operating-point value, and by under $1.7\%$ on all but the single lowest-dose trace; independent
GARP-consistency re-verification fails on 3 of the 17 traces at the smallest margin tested
($\gamma=10^{-6}$), so we do not recommend $\gamma$ below $10^{-4}$.
